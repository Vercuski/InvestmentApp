...
ORDER by
    ...
    public async Task<IEnumerable<TradeSignalPointPoco>> GetLatestBuyTradeSignalPointsAsync()
    {
        using var connection = _connectionFactory.CreateReadConnection();
        return await connection.QueryAsync<TradeSignalPointPoco>(GetLatesTradeSignalPointsSQL, new { actionType = "buy" });
    }

    public async Task<IEnumerable<TradeSignalPointPoco>> GetLatestSellTradeSignalPointsAsync()
    {
        using var connection = _connectionFactory.CreateReadConnection();
        return await connection.QueryAsync<TradeSignalPointPoco>(GetLatesTradeSignalPointsSQL, new { actionType = "sell" });
    }
}
